...
lotus::ArrayValue::ArrayValue(const std::vector<Value>& elements) : elements(elements) {
    type = STRING_LITERAL("array");

    declareMethod(STRING_LITERAL("push"), METHOD(AccessModifierType::PUBLIC, [&](Module& module) {
    ...
    declareMethod(STRING_LITERAL("pop"), METHOD(AccessModifierType::PUBLIC, [&](Module& module) {
...
Value lotus::ARRAY(const std::vector<Value>& elements) {
    return MAKE_PTR<ArrayValue>(elements);
}